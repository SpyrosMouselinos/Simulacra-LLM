...
class HumanAgentInteraction(ComposedPhase):
    ...
    def break_cycle(self, phase_env) -> bool:
        if "<INFO> Finished".lower() in phase_env['modification_conclusion'].lower() or phase_env[
            "comments"].lower() == "end":
            return True
        else:
            return False
...
class SuggestVerifyEnd(ComposedPhase):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
    ...
    def break_cycle(self, phase_env) -> bool:
        if not 'Final step' in phase_env or phase_env['Final step'] == 'no':
            return False
        elif phase_env['Final step'] == "yes":
            return True
        else:
            raise ValueError(f"\nHow the fuck did you terminate? Contents of phase env: {phase_env}\n")
