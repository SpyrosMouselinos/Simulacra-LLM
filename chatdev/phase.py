...
class Phase(ABC):
    ...
    def __init__(self,
        ...
        self.ceo_prompt = role_prompts["Euclid_valid"]  # role_prompts["Chief Executive Officer"]
        self.counselor_prompt = role_prompts["Mathematician_valid"]  # role_prompts["Counselor"]
...
class ProblemPlaning(Phase):
    """
        Storing task into task_prompt
        Storing steps into ideas
    """
    ...
    def update_phase_env(self, chat_env):
        ...
        self.phase_env.update({"task": chat_env.env_dict['task_prompt'], "steps": chat_env.env_dict['ideas']})

    def update_chat_env(self, chat_env) -> ChatEnv:
        if len(self.seminar_conclusion) > 0:
            chat_env.env_dict['ideas'] += self.seminar_conclusion.replace(".", "").strip() + '\n'
        else:
            chat_env.env_dict['ideas'] += "I have no idea where to start.\n"
        return chat_env
    ...
    def execute(self, chat_env, chat_turn_limit, need_reflect) -> ChatEnv:
        ...
        """
        ### Make Euclid oblivious to tool usage ###
        self.update_phase_env(chat_env)
        self.seminar_conclusion = \
            self.chatting(chat_env=chat_env,
                          task_prompt=chat_env.env_dict['task_prompt'],
...
class ProblemPlaningSummary(Phase):
    """
        Storing task into task_prompt
        Storing steps into ideas
    """
    ...
    def update_chat_env(self, chat_env) -> ChatEnv:
        if len(self.seminar_conclusion) > 0:
            ### We must end just at steps, because more things might be added ###
            ri = self.seminar_conclusion.rfind("<STEP>")
            li = self.seminar_conclusion[ri:].find("\n")
            if li == -1:
                li = 0
            seminar_conclusion = self.seminar_conclusion[:ri+li]
            chat_env.env_dict['ideas'] = seminar_conclusion
        else:
            pass
        return chat_env
...
class ToolProblemPlaning(Phase):
    """
        Storing task into task_prompt
        Storing steps into ideas
    """
...
class StepValidation(Phase):
    ...
    def update_phase_env(self, chat_env):
        self.phase_env.update({"task": chat_env.env_dict['task_prompt'],
                               "steps": chat_env.env_dict['ideas']})

    def update_chat_env(self, chat_env) -> ChatEnv:
        if len(self.seminar_conclusion) > 0 and "<VALIDATE>" in self.seminar_conclusion:
            agreement = self.seminar_conclusion.split("<VALIDATE>")[-1]
            if '[Yes]' in agreement or '[yes]' in agreement or 'Yes' in agreement or 'yes' in agreement:
                wrong_step = None
                cont = True
            elif '[No]' in agreement or '[no]' in agreement or 'No' in agreement or 'no' in agreement:
                cont = False
                if '<Step ' in agreement or 'Step ' in agreement or '<step ' in agreement or 'step' in agreement:
                    wrong_step = agreement.split('tep ')[-1][0]
                else:
                    wrong_step = None
            else:
                wrong_step = None
            ...
            if wrong_step is not None:
                wrong_step_loc = chat_env.env_dict['ideas'].find(wrong_step)
                cuttoff_steps = chat_env.env_dict['ideas'][:wrong_step_loc]
                chat_env.env_dict['ideas'] = cuttoff_steps
            else:
                pass
        return chat_env
